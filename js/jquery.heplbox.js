/* HEPL RIA 2013 - Cours
 *
 * JS Document - /js/jquery.heplbox.js
 *
 * coded by leny
 * started at 22/09/13
 */

/* jshint boss: true, curly: true, eqeqeq: true, eqnull: true, immed: true, latedef: true, newcap: true, noarg: true, browser: true, jquery: true, noempty: true, sub: true, undef: true, unused: true, white: false */

;( function( $ ) {
    "use strict";
    //Vérification du code par les navigateurs reconnaissant ce ligne (mode strict)

    $.fn.heplbox = function( oGivenSettings ) {

        var iCurrentIndex = 0,
        // définition de variable et attribution de la valeur 0
            aElements = [],
        // création d'un tableau vie (pour l'instant)
            oDefaultSettings = {
                loaderGifURL: "data:image/gif;base64,",
                cssPrefix: "heplbox-",
                overlayOpacity: 0.66,
                zIndexBase: 9999
            },
        ///définition de valeurs par défault, toujours dans un objet
            oSettings = $.extend( oDefaultSettings, oGivenSettings ),
        //regroupe les deux objets passé en arguments, dans un objet oSettings
            $overlay,
            $container;
        //définition de deux objets viiiiiide

        var setup = function() {
            if( !( $overlay = $( 'body > div.' + oSettings.cssPrefix + 'overlay' ) ).size() ) {
                // si le div ayant la classe "heplbox overlay" n'existe pas
                $overlay = $( '<div class="' + oSettings.cssPrefix + 'overlay"></div>' ).css( {
                    // Alors on crée cee div et on le style en css
                    width: "100%",
                    height: "100%",
                    position: "fixed",
                    top: 0,
                    left: 0,
                    background: "black",
                    zIndex: oSettings.zIndexBase,
                    cursor: 'pointer'
                } ).hide().appendTo( 'body' ).on( 'click.heplbox', closeBox );
                // cache cet élément /  le place a la fin du body / et lui rajoute un évenement au click pour fermer la boite
            }
            if( !( $container = $( 'body > div.' + oSettings.cssPrefix + 'container' ) ).size() ) {
                $container = $( '<div class="' + oSettings.cssPrefix + 'container"></div>' ).css( {
                    width: 50,
                    height: 50,
                    position: "absolute",
                    padding: 5,
                    zIndex: oSettings.zIndexBase + 1,
                    top: 0,
                    left: 0,
                    bottom: 0,
                    right: 0,
                    margin: "auto",
                    background: "white url( " + oSettings.loaderGifURL + " ) center center no-repeat",
                    borderRadius: 5,
                    boxShadow: "0 0 10px rgba( 0, 0, 0, 0.95 )"
                } ).hide().appendTo( 'body' );
                // Même chose avec container sauf qu'on ne lui ajoute aucun événement
            }
        };

        var elementClicked = function( e ) {
            e.preventDefault();
            //annule l'évenement
            iCurrentIndex = aElements.indexOf( $( this ).attr( 'href' ) );
            // cherche l'endroit ou se trouve l'attribut href  dans les image et récupère ces attributs dans iCurrentIndex
            openBox();
            //appelle la fonction openBox
        };

        var openBox = function() {
            $( window ).scrollTop( 0 );
            // doonne la position verticale du scroll de l'utilisateur
            $( "#wrapper" ).css( "-webkit-filter", "blur(3px)" );
            //on floute le div #wrapper via un style css
            $overlay.fadeTo( 'fast', oSettings.overlayOpacity );
            //fade to sur le div contenu dans $overlay
            $container.css( {
                width: 50,
                height: 50,
            } ).fadeIn( 'normal', function() {
                loadImage();
            } );
            //attribution de style css a $contaner et un fadeIn sur l'appel de fonction loadImage
        };

        var loadImage = function() {
            var $img = $( '<img src="' + aElements[ iCurrentIndex ] + '" />' ).css( {
                display: 'inline-block',
                borderRadius: '5px',
                cursor: 'pointer'
                // crée la balise img dans une variable et lui attribue du style css
            } ).hide().appendTo( 'body' ).load( function() {
                // Cache $img / le place a la fin du body / et appele a son chargement les lignes suivantes
                $container.animate( {
                    width: $img.width(),
                    height: $img.height(),
                    // anime le containeur lorsqu'il prend la hauteur et la largeur de l'img
                }, 'fast', function() {
                    $img.remove().appendTo( $container ).fadeIn( 'fast' ).on( 'click.heplbox', nextImage );
                } );
                    // supprime l'élément au click en fadeIn
            } );
        };

        var closeBox = function() {
            $container.fadeOut( 'fast' );
            $( "#wrapper" ).css( "-webkit-filter", "blur(0)" );
            $overlay.fadeOut( 'normal', function() {
                $container.find( 'img' ).remove();
                // supprime le container en fadeOut en utilisant le flou de css3
            } );
        };

        var nextImage = function() {
            if( aElements.length === 1 ) {
                //si le tableau d'image n'en contient qu'une seule, et que l'on passe a l'image suivante, on ferme la fenetre
                return closeBox();
            }
            iCurrentIndex = iCurrentIndex + 1 < aElements.length ? iCurrentIndex + 1 : 0;
            // écriture racourcie de la boucle if... si iCurrentIndex + 1 est plus petit que la longeur de aElements, iCurrentIndex = iCurrentIndex+1, si non, iCurrentIndex = 0
            $container.find( 'img' ).fadeOut( 'fast', function() {
                $container.find( 'img' ).remove();
                loadImage();
                // l'image s'en va, et on passe a load image pour charger la suivante.
            } );
        };

        setup();
        // appelle setup

        return this.each( function() {
            aElements.push( $( this ).attr( 'href' ) );
            $( this ).on( 'click.heplbox', elementClicked );
        } );
    };

} ).call( this, jQuery );
